...
})({"hRDR2":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "474aff87796ad1e0";
"use strict";
...
},{}],"8zppQ":[function(require,module,exports,__globalThis) {
function initCarModal() {
    ...
    carModalTl.fromTo(carModal, {
        maskPosition: "0% 0%"
    }, {
        maskPosition: "50% 0%",
        duration: 1,
        ease: "expo.inOut"
    }).fromTo(carModalClose, {
        y: "-8rem"
    }, {
        y: "0rem",
        duration: 1,
        ease: "expo.inOut"
    }, 0.5).fromTo(carModalGallery, {
        opacity: 0
    }, {
        opacity: 1,
        duration: 1,
        ease: "expo.inOut"
    }, .5).fromTo(carModalThumbs.querySelectorAll(".car-gallery_thumb-wrapper"), {
        y: "12rem"
    }, {
        y: "0rem",
        duration: 1,
    ...
    carModalTrigger.addEventListener("click", ()=>{
        if (carModalTl.progress() === 1) carModalTl.reverse();
        else carModalTl.play();
    });
...
function setupCarGallery(carGallerySource) {
    ...
    items.forEach((item)=>{
        gallery.appendSlide(`<div class="swiper-slide car-gallery_img-wrapper"><img src="${item.url}" class="car-gallery_img"></div>`);
    });
    gallery.update();
}
function getJSONValues(el) {
    // Find the <script> tag inside the <a> tag
    const scriptElement = el.querySelector('script[type="application/json"]');
    if (scriptElement) {
    ...
    } else console.error("No script tag with type application/json found inside the link element.");
}
document.addEventListener("DOMContentLoaded", initCarModal);

},{}]},["hRDR2","8zppQ"], "8zppQ", "parcelRequire94c2")
